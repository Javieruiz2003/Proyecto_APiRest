...
public class RestServer extends AbstractVerticle {
    ...
    public void start(Promise<Void> startFuture) {
        ...
        //Crear un endpoint específico en la API de bajo nivel
    ...
    private void addSensorValue(RoutingContext ctx) {
        ...
        mySqlClient.preparedQuery(
            "INSERT INTO proyectodad.sensorValue (id_sensor,valor,timeStamp) VALUES (?, ?, ?);")
            .execute(Tuple.of(
            	body.getString("id_sensor"),
                body.getString("valor"),
                body.getString("timeStamp")),
            res -> {
                if (res.succeeded()) {
                    // Devuelve el ID del sensorValue creado (opcional pero útil para el cliente)
                    ctx.response()
                        .setStatusCode(201)
                        .putHeader("Content-Type", "application/json")
                        .end(new JsonObject()
                            .put("message", "SensorValue creado correctamente")
