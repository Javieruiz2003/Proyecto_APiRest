...
public class RestServer extends AbstractVerticle {
	...
	public void start(Promise<Void> startFuture) {
		...
		// Crear un endpoint espec�fico en la API de bajo nivel
		router.get("/api/business/sensor/:id_sensor/group").handler(this::getSensorGroup);
		// En tu API de bajo nivel (por ejemplo, RestServer.java)
		router.get("/api/sensors/:id/group").handler(this::getSensorGroup);
	...
	// ===== M�TODOS PARA MANEJAR RUTAS =====

	/** SENSORES **/

	private void getAllSensores(RoutingContext ctx) {
		mySqlClient.query("SELECT * FROM proyectodad.sensor;").execute(res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				JsonArray sensores = new JsonArray();
				for (Row row : resultSet) {
					Sensor sensor = new Sensor(row.getInteger("id"), row.getString("nombre"), row.getString("tipo"),
							row.getString("identificador"), row.getInteger("id_dispositivo"));
					sensores.add(JsonObject.mapFrom(sensor)); // Convierte el objeto Sensor a JSON
				}
				ctx.response().setStatusCode(200).putHeader("Content-Type", "application/json").end(sensores.encode());
			} else {
				ctx.response().setStatusCode(500).end("Error al consultar sensores: " + res.cause().getMessage());
			}
		});
	}
	...
	private void getSensorById(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("SELECT * FROM proyectodad.sensor WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				if (resultSet.size() == 0) {
				...
				ctx.response().setStatusCode(500).end("Error al consultar el sensor: " + res.cause().getMessage());
			}
		});
	}
	...
	private void addSensor(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery(
				...
				.execute(Tuple.of(body.getString("nombre"), body.getString("tipo"), body.getString("identificador"),
						body.getInteger("id_dispositivo")), res -> {
							if (res.succeeded()) {
								// Devuelve el ID del sensor creado (opcional pero �til para el cliente)
								ctx.response().setStatusCode(201).putHeader("Content-Type", "application/json")
										.end(new JsonObject().put("message", "Sensor creado correctamente")

												.encode());
							} else {
								// Manejo espec�fico de errores de SQL (ej: clave for�nea inv�lida)
								if (res.cause().getMessage().contains("foreign key constraint")) {
									ctx.response().setStatusCode(400).end("Error: El id_dispositivo no existe");
								} else {
	...
	private void deleteSensor(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("DELETE FROM proyectodad.sensor WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				ctx.response().setStatusCode(200).end("Sensor eliminado");
			} else {
				ctx.response().setStatusCode(500).end("Error al eliminar: " + res.cause().getMessage());
			}
		});
	}
	...
	private void updateSensor(RoutingContext ctx) {
		...
		mySqlClient
				...
				.execute(Tuple.of(body.getString("nombre"), body.getString("tipo"), body.getString("identificador"),
						body.getInteger("id_dispositivo"), id), res -> {
							if (res.succeeded()) {
								ctx.response().setStatusCode(200).putHeader("Content-Type", "application/json")
	...
	private void getAllActuador(RoutingContext ctx) {
		mySqlClient.query("SELECT * FROM proyectodad.actuador;").execute(res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
	...
	private void getActuadorById(RoutingContext ctx) {
		String id = ctx.pathParam("id"); // Obtiene el ID del path
		mySqlClient.preparedQuery("SELECT * FROM proyectodad.actuador WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				if (resultSet.size() == 0) {
				...
				ctx.response().setStatusCode(500).end("Error al consultar el actuador: " + res.cause().getMessage());
			}
		});
	}
				...
				.execute(Tuple.of(body.getString("nombre"), body.getString("tipo"), body.getString("identificador"),
						body.getInteger("id_dispositivo")), res -> {
							if (res.succeeded()) {
								// Devuelve el ID del sensor creado (opcional pero �til para el cliente)
								ctx.response().setStatusCode(201).putHeader("Content-Type", "application/json")
										.end(new JsonObject().put("message", "Actuador creado correctamente")
	...
	private void deleteActuador(RoutingContext ctx) {
		int id = Integer.parseInt(ctx.pathParam("id"));
		mySqlClient.preparedQuery("DELETE FROM proyectodad.actuador WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				ctx.response().setStatusCode(200).end("Actuador eliminado");
			} else {
				ctx.response().setStatusCode(500).end("Error al eliminar: " + res.cause().getMessage());
			}
		});
	...
	private void getAllDispositivos(RoutingContext ctx) {
		mySqlClient.query("SELECT * FROM proyectodad.dispositivo;").execute(res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
	...
	private void getDispositivoById(RoutingContext ctx) {
		String id = ctx.pathParam("id"); // Obtiene el ID del path
		mySqlClient.preparedQuery("SELECT * FROM proyectodad.dispositivo WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				if (resultSet.size() == 0) {
					// Si no se encuentra el sensor, retorna 404
				...
				ctx.response().setStatusCode(500).end("Error al consultar el dispositivo: " + res.cause().getMessage());
			}
		});
	}
	...
	private void addDispositivo(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("INSERT INTO proyectodad.dispositivo (nombre, id_grupo) VALUES (?, ?);")
				.execute(Tuple.of(body.getString("nombre"), body.getString("id_grupo")), res -> {
					if (res.succeeded()) {
						// Devuelve el ID del dispositivo creado (opcional pero �til para el cliente)
						ctx.response().setStatusCode(201).putHeader("Content-Type", "application/json")
								.end(new JsonObject().put("message", "Dispositivo creado correctamente")
	...
	private void deleteDispositivo(RoutingContext ctx) {
		int id = Integer.parseInt(ctx.pathParam("id"));
		mySqlClient.preparedQuery("DELETE FROM proyectodad.dispositivo WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				ctx.response().setStatusCode(200).end("Dispositivo eliminado");
			} else {
				ctx.response().setStatusCode(500).end("Error al eliminar: " + res.cause().getMessage());
			}
		});
	...
	private void updateDispositivo(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("UPDATE dispositivo SET nombre = ?, id_grupo = ? WHERE id = ?")
				.execute(Tuple.of(body.getString("nombre"), body.getString("id_grupo"), id), res -> {
					if (res.succeeded()) {
						ctx.response().setStatusCode(200).putHeader("Content-Type", "application/json")
								.end(new JsonObject().put("status", "updated").encode());
					} else {
	...
	private void getAllGrupos(RoutingContext ctx) {
		mySqlClient.query("SELECT * FROM proyectodad.grupo;").execute(res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				JsonArray grupos = new JsonArray();
				for (Row row : resultSet) {
					Grupo grupo = new Grupo(row.getInteger("id"), row.getString("canal_mqtt"), row.getString("nombre"));
					grupos.add(JsonObject.mapFrom(grupo)); // Convierte el objeto Grupo a JSON
				}
				ctx.response().setStatusCode(200).putHeader("Content-Type", "application/json").end(grupos.encode());
			} else {
				ctx.response().setStatusCode(500).end("Error al consultar grupos: " + res.cause().getMessage());
			}
		});
	}
	...
	private void getGrupoById(RoutingContext ctx) {
		String id = ctx.pathParam("id"); // Obtiene el ID del path
		mySqlClient.preparedQuery("SELECT * FROM proyectodad.grupo WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				if (resultSet.size() == 0) {
				...
				ctx.response().setStatusCode(500).end("Error al consultar el grupo: " + res.cause().getMessage());
			}
		});
	}
	...
	private void addGrupo(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("INSERT INTO proyectodad.grupo (canal_mqtt,nombre) VALUES (?, ?);")
				.execute(Tuple.of(body.getString("canal_mqtt"), body.getString("nombre")), res -> {
					if (res.succeeded()) {
						// Devuelve el ID del grupo creado (opcional pero �til para el cliente)
						ctx.response().setStatusCode(201).putHeader("Content-Type", "application/json")
								.end(new JsonObject().put("message", "Grupo creado correctamente")
	...
	private void deleteGrupo(RoutingContext ctx) {
		int id = Integer.parseInt(ctx.pathParam("id"));
		mySqlClient.preparedQuery("DELETE FROM proyectodad.grupo WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				ctx.response().setStatusCode(200).end("Grupo eliminado");
			} else {
				ctx.response().setStatusCode(500).end("Error al eliminar: " + res.cause().getMessage());
			}
		});
	...
	private void updateGrupo(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("UPDATE grupo SET  canal_mqtt = ?, nombre = ? WHERE id = ?")
				.execute(Tuple.of(body.getString("canal_mqtt"), body.getString("nombre"), id), res -> {
					if (res.succeeded()) {
						ctx.response().setStatusCode(200).putHeader("Content-Type", "application/json")
								.end(new JsonObject().put("status", "updated").encode());
					} else {
	...
	private void getSensorValueById(RoutingContext ctx) {
		String id = ctx.pathParam("id"); // Obtiene el ID del path
		mySqlClient.preparedQuery("SELECT * FROM proyectodad.sensorValue WHERE id = ?;").execute(Tuple.of(id), res -> {
			if (res.succeeded()) {
				RowSet<Row> resultSet = res.result();
				if (resultSet.size() == 0) {
				...
				ctx.response().setStatusCode(500).end("Error al consultar el sensorValue: " + res.cause().getMessage());
			}
		});
	}
	...
	private void addSensorValue(RoutingContext ctx) {
		...
		mySqlClient.preparedQuery("INSERT INTO proyectodad.sensorValue (id_sensor,valor,timeStamp) VALUES (?, ?, ?);")
				.execute(Tuple.of(body.getString("id_sensor"), body.getString("valor"), body.getString("timeStamp")),
						res -> {
							if (res.succeeded()) {
								// Devuelve el ID del sensorValue creado (opcional pero �til para el cliente)
								ctx.response().setStatusCode(201).putHeader("Content-Type", "application/json")
										.end(new JsonObject().put("message", "SensorValue creado correctamente")
	...
	private void getActuatorStateById(RoutingContext ctx) {
		String id = ctx.pathParam("id"); // Obtiene el ID del path
		mySqlClient.preparedQuery("SELECT * FROM proyectodad.actuatorState WHERE id = ?;").execute(Tuple.of(id),
				res -> {
					if (res.succeeded()) {
						RowSet<Row> resultSet = res.result();
						if (resultSet.size() == 0) {
							...
							// Convierte la fila a un objeto sensorValue y luego a JSON
							Row row = resultSet.iterator().next();
							ActuadorState actuatorState = new ActuadorState(row.getInteger("id"),
									row.getInteger("id_actuador"), row.getBoolean("estado"), row.getLong("timeStamp"));
							ctx.response().setStatusCode(200).putHeader("Content-Type", "application/json")
	...
	private void addActuatorState(RoutingContext ctx) {
		...
		mySqlClient
				.preparedQuery("INSERT INTO proyectodad.actuatorState (id_actuador,estado,timeStamp) VALUES (?, ?, ?);")
				.execute(Tuple.of(body.getString("id_actuador"), body.getString("estado"), body.getString("timeStamp")),
						res -> {
							if (res.succeeded()) {
								// Devuelve el ID del actuatorState creado (opcional pero �til para el cliente)
								ctx.response().setStatusCode(201).putHeader("Content-Type", "application/json")
										.end(new JsonObject().put("message", "ActuatorState creado correctamente")
	...
	private void getSensorGroup(RoutingContext ctx) {
		int sensorId = Integer.parseInt(ctx.pathParam("id"));

		String sql = "";

		mySqlClient.preparedQuery(sql).execute(Tuple.of(sensorId), res -> {
			if (res.succeeded() && res.result().size() > 0) {
				Row row = res.result().iterator().next();
				JsonObject grupo = new JsonObject().put("id", row.getInteger("id"))
						.put("nombre", row.getString("nombre")).put("canal_mqtt", row.getString("canal_mqtt"));

				ctx.response().putHeader("content-type", "application/json").end(grupo.encode());
			} else {
				ctx.response().setStatusCode(404).end("Grupo no encontrado para el sensor");
			}
		});
