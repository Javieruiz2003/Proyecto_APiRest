...
public class MainVerticle extends AbstractVerticle {
	...
	public void start(Promise<Void> startFuture) {
		...
		addSensor(nuevoSensor, res -> {
		    if (res.succeeded()) {
		        System.out.println("Operaci�n completada con �xito");
		    } else {
	...
	private void addSensor(Sensor sensor, Handler<AsyncResult<Void>> handler) {
	    ...
	    mySqlClient.preparedQuery(query, params, res -> {
	        if (res.succeeded()) {
	            System.out.println("Sensor a�adido correctamente");
	            handler.handle(Future.succeededFuture());
	        } else {
	            System.out.println("Error al a�adir sensor: " + res.cause().getLocalizedMessage());
	    ...
	    mySqlClient.preparedQuery(query, Tuple.of(sensorId), res -> {
	        if (res.succeeded()) {
	            ...
	            if (resultSet.rowCount() > 0) {
	                ...
	                System.out.println("No se encontr� el sensor con ID: " + sensorId);
